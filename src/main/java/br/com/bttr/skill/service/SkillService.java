...
public class SkillService {
  ...
  private String validateName(String raw, Long ignoreId) {
    ...
    if (name.length() < 2)
      throw new ApiException(400, "nome de habilidade deve conter pelo menos 2 caracteres.");
    SkillEntity existing =
        repository.findByNameIgnoreCase(currentUser.get().id, name.toLowerCase(Locale.ROOT));
    if (existing != null && !existing.id.equals(ignoreId))
      throw new ApiException(409, "nome de habilidade já existente.");
    return name;
  }
