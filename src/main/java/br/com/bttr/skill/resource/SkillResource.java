...
public class SkillResource {
    ...
    @GET @Path("/skills_from_user")
    @Operation(summary = "Listar todas as habilidades do usuário")
    public SkillsResponse all() { return new SkillsResponse(service.all()); }
    @GET @Path("/skills_by_page")
    @Operation(summary = "Paginar habilidades (5 por página, começando em 1)")
    public PageResponse<SkillView> page(@QueryParam("page") @DefaultValue("1") int page, @Context UriInfo uri) {
        return Pagination.response(service.page(page), page, uri);
    }
    @GET @Path("/skill_by_id/{id}")
    @Operation(summary = "Consultar habilidade do usuário")
    public SkillResponse one(@PathParam("id") @Positive Long id) { return new SkillResponse(service.one(id)); }
    @POST @Path("/create_skill")
    ...
    @PUT @Path("/update_skill_by_id/{id}")
    @Operation(summary = "Atualizar habilidade")
    public MessageResponse update(@PathParam("id") @Positive Long id, @NotNull @Valid SkillInput input) {
        service.update(id, input);
        return new MessageResponse("habilidade alterada com sucesso.");
    }
    @DELETE @Path("/delete_skill_by_id/{id}")
