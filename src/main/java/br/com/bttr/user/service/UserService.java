...
public class UserService {
    ...
    private String email(String email) { return email.trim().toLowerCase(Locale.ROOT); }

    private String username(String username) {
        String result = username.trim();
        if (result.length() < 2) throw new ApiException(400, "nome de usuário deve conter pelo menos 2 caracteres.");
        return result;
    }

    public void signUp(SignUp input) {
        String subject = identities.create(username(input.username()), email(input.email()), input.password());
        try {
            ...
            try { identities.delete(subject); }
            catch (RuntimeException compensation) { LOG.errorf("Falha ao compensar cadastro Keycloak %s", subject); }
            throw failure;
        }
    }
    public SignInResponse signIn(SignIn input) {
        var login = identities.authenticate(email(input.email()), input.password());
        UserEntity user = users.findByKeycloakId(login.profile().id())
                .orElseThrow(() -> new ApiException(401, "usuário não foi encontrado."));
        return new SignInResponse(login.token(), mapper.toView(user, login.profile()), "autenticação realizada com sucesso.");
    }
    ...
    public void update(ProfileUpdate input) {
        identities.update(currentUser.get().keycloakId, username(input.username()), email(input.email()));
    }
    ...
    public void redefine(RedefinePassword input) {
        ...
        if (!user.keycloakId.equals(login.profile().id())) throw new ApiException(401, "senha preenchida é incorreta.");
        identities.changePassword(user.keycloakId, input.newPassword());
    }

    public void forgot(ForgotPassword input) { identities.sendPasswordReset(email(input.email())); }
}
