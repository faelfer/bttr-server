...
public class KeycloakIdentityProvider implements IdentityProvider {
    ...
    @ConfigProperty(name = "bttr.keycloak.url") String server;
    @ConfigProperty(name = "bttr.keycloak.realm") String realm;
    @ConfigProperty(name = "bttr.keycloak.client-id") String clientId;
    @ConfigProperty(name = "bttr.keycloak.client-secret") String clientSecret;
    private final HttpClient http = HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(5)).build();

    private String oidc() { return server + "/realms/" + encode(realm) + "/protocol/openid-connect"; }
    private String users() { return server + "/admin/realms/" + encode(realm) + "/users"; }
    private static String encode(String value) { return URLEncoder.encode(value, StandardCharsets.UTF_8); }

    private HttpResponse<String> request(String method, String url, String token, String body, String contentType) {
    ...
    private ApiException unavailable() { return new ApiException(503, "serviço de autenticação indisponível."); }
    ...
    private void success(HttpResponse<String> response) {
        ...
        if (response.statusCode() == 409) {
            ...
            throw new ApiException(409, message.contains("email") ? "usuário com e-mail já existente." : "nome de usuário já existente.");
        }
        if (response.statusCode() == 404) throw ApiException.notFound("usuário");
        if (response.statusCode() == 400) throw new ApiException(400, "dados rejeitados pelo serviço de autenticação.");
        throw unavailable();
    }
    ...
    private HttpResponse<String> admin(String method, String url, Object body) {
        var response = request(method, url, adminToken(), body == null ? null : serialize(body), "application/json");
        success(response);
        return response;
    }
    ...
    public String create(String username, String email, String password) {
        var body = Map.of("username", username, "email", email, "firstName", username, "lastName", "Bttr", "enabled", true, "emailVerified", false,
    ...
    public Login authenticate(String email, String password) {
        var response = token(Map.of("grant_type", "password", "username", email, "password", password, "scope", "openid profile email"));
        ...
        return new Login(accessToken, new Profile(user.path("sub").asText(),
                user.path("preferred_username").asText(), user.path("email").asText()));
    }
    ...
    public Profile profile(String subject) {
        ...
        return new Profile(user.path("id").asText(), user.path("username").asText(), user.path("email").asText());
    }
    @Override
