...
class AnonymousAccessTest {
    ...
    void allowsAngularDevelopmentOrigin() {
        given().header("Origin", "http://localhost:4200")
                ...
                .post("/users/sign_in").then().statusCode(400)
                .header("Access-Control-Allow-Origin", "http://localhost:4200");
    }

    @Test
    void requiresAuthentication() {
        for (String path : new String[]{"/users/profile", "/skills/skills_from_user", "/skills/skills_by_page", "/times/times_by_page"}) {
    ...
    void validatesPublicEndpointsAndIgnoresStaleClientHeaderOnLogin() {
        given().header("Authorization", "Token null").contentType("application/json").body("{}")
                .post("/users/sign_in").then().statusCode(400);
        given().contentType("application/json").body("{}")
                .post("/users/sign_up").then().statusCode(400);
    }
    @Test
    void publishesOpenApiAndHealth() {
        given().accept("application/json").get("/q/openapi").then().statusCode(200)
